...
class AccountFinishCreateView(
    ...
    def get(self, request, *args, **kwargs):
        # self.object = self.get_object(uuid=request.GET['uuid'])
        # self.object = self.get_object(uuid=request.GET.get('uuid'))
        self.object = self.get_queryset().get(uuid=self.request.GET['UUID'])
...
class PrivacyPolicyView(
    ...
    permission_required = APP_NAME + '.view_account'
    template_name = 'ebay_api/privacy_policy.html'